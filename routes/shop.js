...
router.post('/review/add', auth, (req, res) => {
    ...
    if (req.body.tos && req.body.idprodotto.length > 0 && req.body.username.length > 0 && req.body.titolo.length > 0 && req.body.corpo.length > 0) {
        reviews.push({ idprodotto: req.body.idprodotto, username: req.body.username, titolo: req.body.titolo, corpo: req.body.corpo })
        ...
        res.status(400).send('C\'è stato un problema, assicurati di aver effettuato il login e di aver riempito tutti i campi<br><a href="/shop">Torna indietro</a>')
    }
})
